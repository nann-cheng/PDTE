...
async def async_main(_id):
    ...
    pool.add_http_client(
        "server" + str((_id + 1) % 3),
        addr=SERVER_IPS[(_id + 1) % 3],
        port=NEWTWORK_PORTS[(_id + 1) % 3],
    )
    pool.add_http_client(
        "server" + str((_id + 2) % 3),
        addr=SERVER_IPS[(_id + 2) % 3],
        port=NEWTWORK_PORTS[(_id + 2) % 3],
    )
    ...
    if BENCHMARK_MEASURE_OFFLINE_COMMU:
        ...
        if _id == 0:
            ...
            print(
                "\n*********Offline FeatSelect preparation communication cost's:",
                nowRecv - lastRecv,
                " bytes!*********\n",
            )
            lastRecv = nowRecv
        ...
        if _id == 0:
            ...
            nowRecv = pool.http_server.total_bytes_recv
            print(
                "\n*********Offline PathEval preparation communication cost's: ",
                nowRecv - lastRecv,
                " bytes!*********\n",
            )
            lastRecv = nowRecv
    ...
    if _id == 0:
        ...
        if BENCHMARK_MEASURE_OFFLINE_COMMU:
            ...
            nowRecv = pool.http_server.total_bytes_recv
            print(
                "\n*********Offline Compare preparation communication cost's: ",
                nowRecv - lastRecv,
                "bytes!*********\n",
            )
    ...
    if _id == 0:
        ...
        while success_time < chunks_size:
            ...
            if index in server2_buffer:
                ...
                compute_corotines.append(
                    asyncio.create_task(player.featureSelect1(index, vArray, nSSArray))
                )
                success_time += 1
        ...
        while success_time < chunks_size:
            ...
            compute_corotines.append(
                asyncio.create_task(player.featureSelect1(index, array, None))
            )
            success_time += 1
    elif _id == 2:
        server0_buffer = {}
        ...
        while success_time < chunks_size:
            ...
            if index in server1_buffer:
                ...
                del server1_buffer[index]
                compute_corotines.append(
                    asyncio.create_task(player.featureSelect1(index, vArray, array))
                )
                success_time += 1
                if success_time == chunks_size:
                    break
    ...
    if BENCHMARK_MEASURE_ONLINE_COMPUTATION:
        end_time = time.time()
        print(
            "\n*********Online FeatureSelect computation time's: ",
            end_time - start_time,
            "s!*********\n",
        )
        start_time = end_time
    ...
    if _id == 0:
        while success_time < chunks_size:
            ...
            success_time += 1
            compute_corotines.append(
                asyncio.create_task(player.compare1(index, otherShares))
            )
    elif _id == 1:
        while success_time < chunks_size:
    ...
    if BENCHMARK_MEASURE_ONLINE_COMMU:
        nowRecv = pool.http_server.total_bytes_recv
        print(
            "\n*********Online Compare communication cost's: ",
            nowRecv - lastRecv,
            "bytes!*********\n",
        )
        lastRecv = nowRecv
    ...
    if BENCHMARK_MEASURE_ONLINE_COMPUTATION:
        end_time = time.time()
        print(
            "\n*********Online compare computation time's: ",
            end_time - start_time,
            "s!*********\n",
        )
        start_time = end_time
    ...
    if BENCHMARK_MEASURE_ONLINE_COMMU:
        nowRecv = pool.http_server.total_bytes_recv
        print(
            "\n*********Online PathEval communication cost's: ",
            nowRecv - lastRecv,
            "bytes!*********\n",
        )
        lastRecv = nowRecv
    ...
    if BENCHMARK_MEASURE_ONLINE_COMPUTATION:
        end_time = time.time()
        print(
            "\n*********Online PathEval computation time's: ",
            end_time - start_time,
            "s!*********\n",
        )
        print(
            "\n*********Total online computation time's: ",
    ...
    if BENCHMARK_CHECK_PROTOCOL_CORRECTNESS:
        if _id == 0:
            ...
            print(
                "The final selected class is: ",
                inRing(v00 + v01 + v11, VEC_VAL_MAX_BOUND),
            )
    await pool.shutdown()
